...
type DataTableBulkActionsProps<TData extends BaseRecord> = {
...
export function DataTableBulkActions<TData extends BaseRecord>({
  ...
  useEffect(() => {
    if (selectedCount > 0) {
      const message = `${selectedCount} Row${selectedCount > 1 ? 's' : ''} selected. Bulk actions toolbar is available.`
  ...
  return (
    <>
      ...
      <div
        ref={toolbarRef}
        role='toolbar'
        aria-label={`Bulk actions for ${selectedCount} selected row${selectedCount > 1 ? 's' : ''}`}
        ...
        >
          <Tooltip>
            <TooltipTrigger asChild>
              ...
              >
                <X />
                <span className='sr-only'>取消选中</span>
              </Button>
            </TooltipTrigger>
            <TooltipContent>
              <p>取消选中 (Escape)</p>
